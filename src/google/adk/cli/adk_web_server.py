...
def _setup_telemetry(
    ...
    if otel_to_cloud:
        ...
        _setup_telemetry_from_env_experimental(internal_exporters=internal_exporters)
    else:
...
class AdkWebServer:
    ...
    def get_fast_api_app(
        ...
        setup_observer: Callable[[Observer, "AdkWebServer"], None] = lambda o, s: None,
        ...
        # Add A2A auth middleware first (will execute after OAuth2 middleware)
        app.add_middleware(
        ...
        # Add OAuth2 middleware last (will execute first in the request chain)
        ...
        if self.oauth2_handler:
            ...
            async def oauth2_callback(
                ...
                state_data = self.oauth2_handler.state_store.validate_and_consume_state(
                    state
                )
                if not state_data:
                    raise HTTPException(
            ...
            async def get_current_user_info(request: Request):
                ...
                session = self.oauth2_handler.get_session_from_request(request)

                if not session or session.is_expired():
                    raise HTTPException(status_code=401, detail="Not authenticated")

                if not session.user_info:
                    # Try to fetch user info if not cached
                    if self.oauth2_handler.config.userinfo_url:
                        try:
                            user_info = await self.oauth2_handler._fetch_user_info(session.access_token)
